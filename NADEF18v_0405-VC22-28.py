...
def update_batch_line(event):
    ...
    if value:
        ...
        if batch_line is None:
            ...
            batch_number = f"BachNo:{CURRENT_PRODUCT_LETTER}{year:02d}{month:02d}{day:02d}-{DAILY_BATCH_NUMBER:02d}/"
...
window.title("NADEF18-PRO")
window.resizable(1, 0)
window.config(bg="#133c72")
...
PRODUCTS_CONFIG_DIR = os.path.join(os.environ.get("APPDATA", os.path.expanduser("~")), "NADEF18-PRO")
...
# شماره‌ی batch روزانه (1 تا 10) — با انتخاب کاربر در پنجره‌ی START تعیین می‌شود
# و جایگزین همان دو رقم "-01" انتهای BachNo می‌شود. تا وقتی کاربر عددی انتخاب
# نکرده (یا اگر پنجره را ببندد)، مقدار پیش‌فرض 1 (یعنی -01، دقیقاً مثل قبل) است.
...
def _normalize_time_field(entry_widget):
    """اگر متن داخل این فیلد ساعت شبیه یک ساعت باشد ولی به‌جای ':' از چیز
    دیگری (مثلاً ';') استفاده شده یا اصلاً جداکننده نداشته باشد (مثل '1200'
    یا '12;00')، آن را به فرمت استاندارد 'HH:MM' اصلاح و در همان فیلد
    جایگزین می‌کند. اگر فیلد خالی باشد، از قبل استاندارد (':') باشد، یا اصلاً
    شکل یک ساعت معتبر را نداشته باشد، دست‌نخورده می‌ماند تا رفتار و خطاهای
    قبلی برنامه (مثل پیام‌های Please enter valid time) عوض نشود."""
    ...
    if ':' in cleaned:
        ...
        if not digits.isdigit() or len(digits) not in (3, 4):
            return
        hh_str, mm_str = (digits[0], digits[1:]) if len(digits) == 3 else (digits[:2], digits[2:])
    try:
        hh_i, mm_i = int(hh_str), int(mm_str)
    except ValueError:
        return
    if not (0 <= hh_i <= 23 and 0 <= mm_i <= 59):
        return
    entry_widget.delete(0, END)
    entry_widget.insert(0, f"{hh_i:02d}:{mm_i:02d}")


def allesFunc():
    # پیش از هر محاسبه‌ای، هر فیلد ساعتی که بدون ':' یا با جداکننده‌ی دیگری
    # (مثل ';') نوشته شده باشد (مثل 1200 یا 12;00)، اصلاح می‌شود تا محاسبات
    # زیر با فرمت درست HH:MM انجام شوند.
    _normalize_time_field(entry02)
    for _t in list2:
        _normalize_time_field(_t)
...
# ---------------------------------------------------------
    ...
    counter = 0
    for lm, dose, t0, low, act, vol, te, gb, eid in zip(list0, list1, list2, list3, list4, list5, list6, listGBI, listEID):
    ...
    #    محاسبه اکتیویته پایین
        ...
        sump = 0
        for i in range(req):
            num = eid_val*2**((i*gap_time)/109.771)
            sump += num
...
# توجه: طبق درخواست، فیلد و لیبل G.T از روی صفحه حذف شدند (این Entry دیگر
# grid/نمایش داده نمی‌شود) و دکمه‌ی Clear جایگزین آن‌ها شد (پایین‌تر). اما
# همین entry03 در محاسبات allesFunc (زمان اولین تزریق هر مرکز) همچنان
# استفاده می‌شود — پس خودِ widget را دست‌نخورده نگه می‌داریم تا هیچ محاسبه‌ای
# تغییر نکند. چون دیگر هیچ‌وقت روی صفحه دیده/پر نمی‌شود، همیشه خالی می‌ماند و
# طبق همان منطق قبلیِ allesFunc به‌صورت خودکار مقدار پیش‌فرض 30 دقیقه می‌گیرد.
...
def clear_page_entries():
    """دکمه‌ی Clear: تمام Entry های قابل‌ویرایش این صفحه (ردیف بالا، جدول
    مراکز، GBI، EID، درصد کاهش و جمع‌ها) را پاک می‌کند. لیبل‌های ثابت نام مراکز
    (list0) و دکمه‌های +/- (list8, list9) دست‌نخورده می‌مانند تا هیچ رفتار
    دیگری از برنامه تغییر نکند. سپس پنجره‌ی انتخاب شماره‌ی batch امروز باز
    می‌شود تا کل BachNo روی لیبل بر اساس تاریخ همین امروز به‌روزرسانی شود."""
    for e in listuprow:
        e.delete(0, END)
    for e in list1:
        e.delete(0, END)
    ...
    entry_sum_A.delete(0, END)
    entry_sum_V.delete(0, END)
    _select_daily_batch_number()


btn_clear = Button(frame0, text="Clear", fg="#000000", bg="#F5B301", font=UI_TOPBAR_FONT,
                   width=7, command=clear_page_entries)
btn_clear.grid(row=0, column=7, columnspan=2, padx=2, sticky='ew')

# دکمه‌های Settings / Report / Advance — هر سه داخل یک زیرفریم مستقل قرار
# می‌گیرند تا دقیقاً زیر هم و نزدیک به هم بمانند. قبلاً هرکدام مستقیم در
# frame0 (ردیف‌های 0/1/2، ستون 9) گرید می‌شدند؛ چون frame_comecer (شامل
# ردیف‌های Host/Port و VIK-202/VIK-203) هم در همان ردیف 1 با ارتفاع خیلی
# زیاد قرار داشت، دکمه‌ی Advance (ردیف 2) مجبور بود بعد از تمام آن ارتفاع
# بلند شروع شود و در عمل خیلی پایین‌تر از Settings/Report ظاهر می‌شد. با
# یک زیرفریم مستقل، این سه دکمه دیگر به ارتفاع ردیف‌های دیگر frame0 وابسته
# نیستند و همیشه دقیقاً زیر هم و نزدیک می‌مانند.
UI_TOPBTN_WIDTH = 12
UI_TOPBTN_HEIGHT = 1

frame_topbtns = Frame(frame0, bg='#154360')
frame_topbtns.grid(row=0, column=9, rowspan=3, padx=6, pady=(6, 0), sticky='n')

btn_settings = Button(frame_topbtns, text="\u2699 Settings", fg="#000000", bg="#D5DBDB",
                      font=UI_TOPBAR_FONT, width=UI_TOPBTN_WIDTH, height=UI_TOPBTN_HEIGHT,
                      command=lambda: open_print_settings_window())
btn_settings.grid(row=0, column=0, pady=(0, 4))

btn_report = Button(frame_topbtns, text="Report", fg="#000000", bg="#D5DBDB",
                    font=UI_TOPBAR_FONT, width=UI_TOPBTN_WIDTH, height=UI_TOPBTN_HEIGHT,
                    command=lambda: generate_dispense_report())
btn_report.grid(row=1, column=0, pady=(0, 4))

# دکمه Advance — مدیریت نام محصولات و حرف BachNo؛ حالا دقیقاً زیر Report
# و با همان فاصله‌ای که بین Settings و Report است.
btn_advance = Button(frame_topbtns, text="Advance", fg="#000000", bg="#D5DBDB",
                     font=UI_TOPBAR_FONT, width=UI_TOPBTN_WIDTH, height=UI_TOPBTN_HEIGHT,
                     command=lambda: open_advance_window())
btn_advance.grid(row=2, column=0)
...
def comecer_toggle_activity_lock():
    ...
    if comecer_state["activity_locked"]:
        btn_lock_activity.config(text="Unlock Whole Activity", bg="#D1350E", fg="white")
    else:
        btn_lock_activity.config(text="Lock Whole Activity", bg="#D5DBDB", fg="black")


btn_lock_activity = Button(frame_comecer, text="Lock Whole Activity", bg="#D5DBDB", fg="black",
...
def comecer_handle_reading(reading):
    ...
    if reading.chamber_id == CHAMBER_ID_FOR_VIK202 and not comecer_state["activity_locked"]:
        ...
        #
        # نکته‌ی مهم: allesFunc (تابع دکمه‌ی Run) با entry0.get().isdigit() چک
        # می‌کند که مقدار یک عدد صحیح ساده باشد (چون همیشه فرض بوده کاربر چیزی
        # مثل '5000' تایپ می‌کند). رشته‌ی شامل ممیز اعشار مثل '28.867' باعث
        # False شدن isdigit() می‌شود و کل محاسبات مراکز بی‌صدا رد می‌شوند. برای
        # همین، اینجا هم باید عدد صحیح (بدون اعشار) نوشته شود، نه با ۳ رقم اعشار.
        entry0.delete(0, "end")
        entry0.insert(0, str(int(round(activity_mci))))
...
# ------EID مقدار پایه‌ی هر مرکز برای فرمول اکتیویته (پیش‌فرض 10)------------------
entryEID1 = Entry(frame1, font=UI_FONT_ENTRY_BOLD, bg="#D5DBDB", width=5, justify='center')
entryEID1.grid(row=2, column=0, padx=UI_COL_PAD)
entryEID1.insert(0, "10")

entryEID2 = Entry(frame1, font=UI_FONT_ENTRY_BOLD, bg="#D5DBDB", width=5, justify='center')
entryEID2.grid(row=3, column=0)
entryEID2.insert(0, "10")

entryEID3 = Entry(frame1, font=UI_FONT_ENTRY_BOLD, bg="#D5DBDB", width=5, justify='center')
entryEID3.grid(row=4, column=0)
entryEID3.insert(0, "10")

entryEID4 = Entry(frame1, font=UI_FONT_ENTRY_BOLD, bg="#D5DBDB", width=5, justify='center')
entryEID4.grid(row=5, column=0)
entryEID4.insert(0, "10")

entryEID5 = Entry(frame1, font=UI_FONT_ENTRY_BOLD, bg="#D5DBDB", width=5, justify='center')
entryEID5.grid(row=6, column=0)
entryEID5.insert(0, "10")

entryEID6 = Entry(frame1, font=UI_FONT_ENTRY_BOLD, bg="#D5DBDB", width=5, justify='center')
entryEID6.grid(row=7, column=0)
entryEID6.insert(0, "10")

entryEID7 = Entry(frame1, font=UI_FONT_ENTRY_BOLD, bg="#D5DBDB", width=5, justify='center')
entryEID7.grid(row=8, column=0)
entryEID7.insert(0, "10")

entryEID8 = Entry(frame1, font=UI_FONT_ENTRY_BOLD, bg="#D5DBDB", width=5, justify='center')
entryEID8.grid(row=9, column=0)
entryEID8.insert(0, "10")

entryEID9 = Entry(frame1, font=UI_FONT_ENTRY_BOLD, bg="#D5DBDB", width=5, justify='center')
entryEID9.grid(row=10, column=0)
entryEID9.insert(0, "10")

entryEID10 = Entry(frame1, font=UI_FONT_ENTRY_BOLD, bg="#D5DBDB", width=5, justify='center')
entryEID10.grid(row=11, column=0)
entryEID10.insert(0, "10")

entryEID11 = Entry(frame1, font=UI_FONT_ENTRY_BOLD, bg="#D5DBDB", width=5, justify='center')
entryEID11.grid(row=12, column=0)
entryEID11.insert(0, "10")

entryEID12 = Entry(frame1, font=UI_FONT_ENTRY_BOLD, bg="#D5DBDB", width=5, justify='center')
entryEID12.grid(row=13, column=0)
entryEID12.insert(0, "10")

# ------GBI فواصل بین تزریق مراکز---------------------------------------------------
entryGB1 = Entry(frame1, font=UI_FONT_ENTRY_BOLD,
                bg="#D5DBDB", width=5, justify='center')
entryGB1.grid(row=2, column=1, padx=UI_COL_PAD)
...
label_EID = Label(frame1, text="EID", fg="black",
                    bg='#6A8EB6', font=UI_FONT_HEADER).grid(row=1, column=0)
label_GBI=Label(frame1, text="GBI", fg="black",
                    bg='#6A8EB6', font=UI_FONT_HEADER).grid(row=1, column=1)
label_centername = Label(frame1, text="center name", fg="black",
                    bg='#6A8EB6', font=UI_FONT_HEADER).grid(row=1, column=2)
label_ReqNumber = Label(frame1, text="Req\nNumber", fg="black",
                    bg='#6A8EB6', font=UI_FONT_HEADER).grid(row=1, column=3)
label_FirstInjTime = Label(frame1, text="First\nInj\nTime", fg="black",
                    bg='#6A8EB6', font=UI_FONT_HEADER).grid(row=1, column=4)
label_DeliveryTimeActivity = Label(frame1, text="Delivery\nTime\nActivity", fg="black",
                    bg='#6A8EB6', font=UI_FONT_HEADER).grid(row=1, column=5)
label_DispenseTimeActivity = Label(frame1, text="Dispense\nTime\nActivity", fg="black",
                    bg='#6A8EB6', font=UI_FONT_HEADER).grid(row=1, column=6)
label_NeededVolume = Label(frame1, text="Needed\nVolume", fg="black",
                    bg='#6A8EB6', font=UI_FONT_HEADER).grid(row=1, column=7)
label_NextDispenseTime = Label(frame1, text="Next\nDispense\nTime", fg="black",
                    bg='#6A8EB6', font=UI_FONT_HEADER).grid(row=1, column=8)
...
entry12.grid(row=2, column=4, padx=UI_COL_PAD)
...
entry10_2 = Entry(frame1, font=UI_FONT_TIME,
                bg="#D5DBDB", fg="red", width=5, justify='center')
entry10_2.grid(row=11, column=4, padx=UI_COL_PAD)
...
entry11_2 = Entry(frame1, font=UI_FONT_TIME,
                bg="#D5DBDB", fg="red", width=5, justify='center')
entry11_2.grid(row=12, column=4, padx=UI_COL_PAD)
...
entry12_2 = Entry(frame1, font=UI_FONT_TIME,
                bg="#D5DBDB", fg="red", width=5, justify='center')
entry12_2.grid(row=13, column=4, padx=UI_COL_PAD)
...
# دقیقاً زیر همان ستونی که هر entry در ردیف‌های بالا دارد (column=6 زیر
# «Dispense Time Activity» و column=7 زیر «Needed Volume»)، تا label زیر
# entry بالای خودش بماند، نه در یک زیر-فریم جدا با چیدمان مستقل.
entry_sum_A = Entry(frame1, font=("STENCIL", 12), takefocus=False, width=8, justify='center', bg="#878C92")
entry_sum_A.grid(row=14, column=6, padx=3)
label_sum_A = Label(frame1, text=" Activity Sum ", font=("0 jadid bold", 11, "bold"),
                    bg='#6A8EB6')
label_sum_A.grid(row=15, column=6, pady=0, ipady=0)

entry_sum_V = Entry(frame1,font=("STENCIL", 12), takefocus=False, width=8, justify='center', bg="#878C92")
entry_sum_V.grid(row=14, column=7, padx=3)
label_sum_V = Label(frame1, text=" Volume Sum ", font=("0 jadid bold", 10, "bold"),
                    bg='#6A8EB6')
label_sum_V.grid(row=15, column=7, pady=0, ipady=0)

# دکمه‌های QC1 / QC2 — دقیقاً زیر ستون آخر (Next Dispense Time، column=8) که
# در این دو ردیف (14 و 15) خالی بود، پس اضافه‌شدنشان اندازه یا چیدمان هیچ
# ستون/ردیف دیگری را به هم نمی‌زند.
def apply_qc_suffix(qc_name):
    """با زدن QC1 یا QC2: بخش بعد از '/' در خط BachNo با QC1/QC2 جایگزین
    می‌شود، حجم خط 'container/' روی مقدار پیش‌فرض 0.3 تنظیم می‌شود (که با
    تایپ‌کردن قابل اصلاح است)، و خط Time روی لیبل طوری تنظیم می‌شود که هم
    Time و هم Cal Time دقیقاً برابر همین لحظه‌ی سیستم کامپیوتر باشند — یعنی
    مبنای محاسبه‌ی A-Activity هم همین ساعت سیستم است (بدون افت پرتوزایی بین
    Time و Cal Time، چون هر دو یکی‌اند)."""
    text = text_field.get("1.0", "end-1c")
    lines = text.split('\n')

    # --- خط "... container/ X ml": حجم پیش‌فرض 0.3 ---
    for i, line in enumerate(lines):
        if 'container/' in line and 'ml' in line:
            before = line.split('container/')[0] + 'container/ '
            lines[i] = f"{before}0.3 ml"
            break

    # --- خط BachNo: هرچه بعد از '/' بود با QC1/QC2 جایگزین می‌شود ---
    batch_line = None
    for i, line in enumerate(lines):
        if line.startswith("BachNo:"):
            batch_line = i
    ...
    if batch_line is None:
        ...
        batch_line = len(lines) - 1
    batch_line_content = re.sub(r"(/.*)$", "/", lines[batch_line])
    lines[batch_line] = batch_line_content + qc_name

    # --- خط Time: هم Time و هم Cal Time برابر همین ساعت سیستم می‌شوند ---
    now_str = datetime.now().strftime("%H:%M")
    for i, line in enumerate(lines):
        if line.startswith("Time:"):
            lines[i] = f"Time: {now_str} - Cal Time:{now_str}"
            break
...
frame_qc = Frame(frame1, bg='#6A8EB6')
frame_qc.grid(row=14, column=8, rowspan=2, pady=0)
btn_qc1 = Button(frame_qc, text="QC1", width=6, bg="#D5DBDB",
                 command=lambda: apply_qc_suffix("QC1"))
btn_qc1.pack(side='left', padx=2)
btn_qc2 = Button(frame_qc, text="QC2", width=6, bg="#D5DBDB",
                 command=lambda: apply_qc_suffix("QC2"))
btn_qc2.pack(side='left', padx=2)


# ------------------------------درصد کاهش 

label_enter=Label(frame1,text="apply", font=("0 jadid bold", 11, "bold"),bg='#DA8416')
label_enter.grid(row=14, column=1, padx=0, pady=2)
...
def get_persian_date_code():
    try:
        ...
        return f"BachNo:{CURRENT_PRODUCT_LETTER}000000-{DAILY_BATCH_NUMBER:02d}/"  # fallback if package not installed
    today = JalaliDate.today()
    year = today.year % 100  # last two digits
    month = today.month
    day = today.day
    return f"BachNo:{CURRENT_PRODUCT_LETTER}{year:02d}{month:02d}{day:02d}-{DAILY_BATCH_NUMBER:02d}/"
...
def _apply_daily_batch_number_to_label():
    """کل بخش BachNo (پیشوند محصول + تاریخ امروز + شماره‌ی batch انتخابی، یعنی
    همان چیزی که get_persian_date_code تولید می‌کند) را از نو می‌سازد و در
    خط BachNo فعلیِ روی لیبل جایگزین می‌کند — یعنی تاریخ هم با تاریخ همین
    امروز به‌روزرسانی می‌شود، نه فقط دو رقم شماره‌ی batch. فقط بخش بعد از '/'
    (مثل QC1 یا نام مرکز) دست‌نخورده می‌ماند."""
    text = text_field.get("1.0", "end-1c")
    lines = text.split('\n')
    for i, line in enumerate(lines):
        if line.startswith("BachNo:"):
            ...
            after_slash = tail if sep else ''
            new_batch_line = get_persian_date_code()  # مثل "BachNo:C020050628-03/"
            lines[i] = new_batch_line + after_slash if sep else new_batch_line.rstrip('/')
            text_field.delete("1.0", "end")
            text_field.insert("1.0", '\n'.join(lines))
            break
...
def _send_one_print_copy(printer_name, text):
    """فقط یک نسخه‌ی فیزیکی از متن را به پرینتر ارسال می‌کند (کامل: OpenPrinter تا
    ClosePrinter). این تابع هیچ ثبتی در تاریخچه/گزارش انجام نمی‌دهد — ثبت گزارش
    فقط یک‌بار در print_text() انجام می‌شود تا محتوای تکراری دو بار در فایل
    گزارش dispense ذخیره نشود."""
    hprinter = None
    hdc = None
    try:
        ...
        hdc.StartDoc("Dose Label - NADEF18-PRO")
        hdc.StartPage()
...
PRINT_COPIES_PER_CLICK = 1  # با هر بار زدن دکمه PRINT، همین تعداد نسخه‌ی فیزیکی چاپ ارسال می‌شود
                            # (از طریق دکمه Settings قابل تغییر است؛ برای مثال 1 یا 3)


def print_text():
    """با هر بار زدن دکمه PRINT، PRINT_COPIES_PER_CLICK نسخه (پیش‌فرض 1، قابل تغییر
    از پنجره‌ی Settings) از همان لیبل به پرینتر انتخاب‌شده (COMECER_PRINTER_NAME) ارسال می‌شود.
    ...
    - چون محتوای هر PRINT_COPIES_PER_CLICK نسخه کاملاً یکسان است، در فایل/تاریخچه‌ی
      گزارش dispense report فقط یک‌بار (نه به تعداد نسخه‌های چاپ‌شده) ثبت می‌شود.
    """
    try:
        printer_name = COMECER_PRINTER_NAME or win32print.GetDefaultPrinter()
        text = text_field.get("1.0", END).strip()

        for _ in range(PRINT_COPIES_PER_CLICK):
            _send_one_print_copy(printer_name, text)

        # ثبت متن این لیبل در تاریخچه‌ی همان batch (برای گزارش کامل مراکز پرینت‌شده) —
        # فقط یک‌بار، صرف‌نظر از تعداد نسخه‌های فیزیکی چاپ‌شده، تا محتوای تکراری در
        # فایل گزارش ذخیره نشود.
        batch_key = extract_batch_name(text) or "DispenseReport"
        print_history_by_batch.setdefault(batch_key, []).append(text)

        messagebox.showinfo(
            "Print",
            f"{PRINT_COPIES_PER_CLICK} copies sent to '{printer_name}' successfully."
        )
        return True
...
def _parse_label_fields(entry_text):
    """یک متن لیبل (چندخطی، همان چیزی که در فایل txt ذخیره می‌شود) را به فیلدهای
    جداگانه تفکیک می‌کند تا هرکدام در یک ستون مجزای فایل اکسل نوشته شوند:
    - خط 'BachNo:C020050617-01/QC1' به دو بخش تفکیک می‌شود: BachNo (بخش قبل
      از '/'، مثل C020050617-01) و بخش بعد از '/' (مثل QC1 یا Payam6). این
      بخش دوم خودش به دو ستون دیگر تفکیک می‌شود: Center Name (فقط حروف، مثل
      QC یا Payam) و Dose Number (فقط عدد انتهای آن، مثل 1 یا 6) — با یک
      استثنا: اگر Center Name برابر QC باشد، Dose Number همیشه 0 درج می‌شود.
    - خط 'Time: 09:23 - Cal Time:09:22' فقط ساعتی که بعد از 'Cal Time:' آمده
      (یعنی 09:22) نگه داشته می‌شود — به‌جز برای QC که فقط همان ساعت اول
      (قبل از ' - ') نگه داشته می‌شود، مثلاً از 'Time: 14:36 - --:--' فقط
      '14:36' (چون برای QC معمولاً Cal Time هیچ‌وقت ثبت نمی‌شود).
    - خط '15 ml container/ 0.3 ml' فقط بخش بعد از '/' (0.3 ml) نگه داشته می‌شود.
    """
    fields = {
        "Name": "",
        "Activity": "",
    ...
    for raw_line in entry_text.split("\n"):
        ...
        if not line:
            continue
        if line.startswith("A-Activity:"):
            fields["A-Activity"] = line[len("A-Activity:"):].strip()
        elif line.startswith("Activity:"):
            fields["Activity"] = line[len("Activity:"):].strip()
        elif line.startswith("Name:"):
            fields["Name"] = line[len("Name:"):].strip()
        elif line.startswith("Time:"):
            # نهایی‌کردن Time بعد از پایان حلقه انجام می‌شود، چون تشخیص QC به
            # Center Name نیاز دارد که از خط BachNo (که بعد از خط Time می‌آید)
            # به‌دست می‌آید.
            raw_time_val = line[len("Time:"):].strip()
        elif line.startswith("BachNo:"):
            bach_val = line[len("BachNo:"):].strip()
            bach_part, sep, center_part = bach_val.partition("/")
            fields["BachNo"] = bach_part.strip()
            center_raw = center_part.strip() if sep else ""
            # عدد انتهای متن (تعداد دوز) را از نام مرکز جدا می‌کند، مثلاً از
            # 'Payam6' نام 'Payam' و عدد '6' به‌دست می‌آید. اگر عددی در انتها
            # نبود (مثل حالتی که فقط اسم تایپ شده)، Dose Number خالی می‌ماند.
            m = re.match(r'^(.*?)(\d+)$', center_raw)
            if m:
                fields["Center Name"] = m.group(1).strip()
                fields["Dose Number"] = m.group(2).strip()
            else:
                fields["Center Name"] = center_raw
                fields["Dose Number"] = ""
            # استثنا: وقتی Center Name برابر QC باشد (مثلاً از 'QC1' یا 'QC2')،
            # مقدار Dose Number همیشه 0 درج می‌شود، نه عدد واقعی جلوی آن.
            if fields["Center Name"] == "QC":
                fields["Dose Number"] = "0"
        elif "container/" in line and "ml" in line:
            # فقط بخش بعد از 'container/' نگه داشته می‌شود، مثلاً از
            # '15 ml container/ 0.3 ml' فقط '0.3 ml'
            fields["Container"] = line.split("container/", 1)[1].strip()

    # نهایی‌کردن ستون Time: برای QC فقط ساعت اول (قبل از ' - ') نگه داشته
    # می‌شود؛ برای بقیه، اگر 'Cal Time:' ثبت شده باشد همان مقدار بعد از آن،
    # وگرنه همان مقدار خام نگه داشته می‌شود.
    if raw_time_val is not None:
        if fields["Center Name"] == "QC":
            fields["Time"] = raw_time_val.split(" - ")[0].strip()
        elif "Cal Time:" in raw_time_val:
            fields["Time"] = raw_time_val.split("Cal Time:", 1)[1].strip()
        else:
            fields["Time"] = raw_time_val

    return fields


def _format_entry_for_txt(entry_text):
    """اگر لیبل مربوط به QC باشد (Center Name برابر QC، یعنی بعد از '/' در خط
    BachNo مقداری مثل QC1/QC2/... آمده)، خط Time آن را طوری اصلاح می‌کند که
    فقط همان ساعت اول نوشته شود — مثلاً از 'Time: 14:36 - --:--' فقط
    'Time: 14:36' — نه خط کامل با پسوند '- --:--'. برای بقیه‌ی لیبل‌ها متن
    بدون هیچ تغییری برگردانده می‌شود."""
    fields = _parse_label_fields(entry_text)
    if fields["Center Name"] != "QC":
        return entry_text
    ...
    for raw_line in entry_text.split("\n"):
        ...
        if stripped.startswith("Time:"):
            time_val = stripped[len("Time:"):].strip()
            first_part = time_val.split(" - ")[0].strip()
            new_lines.append(f"Time: {first_part}")
        else:
            new_lines.append(raw_line)
    return "\n".join(new_lines)


# زیرپوشه‌ای که فایل‌های اکسل دائمی (یکی به‌ازای هر Name) در آن نگه‌داری
# می‌شوند. برخلاف فایل txt که به‌ازای هر BachNo از نو ساخته/بازنویسی می‌شود،
# این فایل‌ها فقط یک‌بار به‌ازای هر Name ساخته شده و برای همیشه باقی می‌مانند.
EXCEL_BY_NAME_SUBFOLDER = "Excel by Name"

# تعداد لیبل‌هایی که از هر batch تا الان در فایل‌های اکسل ثبت شده‌اند — تا با
# هر بار کلیک روی دکمه‌ی Report، فقط لیبل‌های تازه (که از دفعه‌ی قبل اضافه
# نشده‌اند) به آخر فایل اکسل اضافه شوند، نه اینکه لیبل‌های قبلی دوباره و
# تکراری درج شوند.
_excel_export_progress_by_batch = {}

_EXCEL_COLUMN_HEADERS = ["#", "Name", "BachNo", "Center Name", "Dose Number", "Activity", "A-Activity", "Time", "Container"]


def _append_entries_to_name_excel(report_dir, entries):
    """لیبل‌های داده‌شده را بر اساس مقدار Name آن‌ها گروه‌بندی می‌کند و هرکدام را
    به انتهای فایل اکسل دائمی همان Name اضافه می‌کند (مسیر: <report_dir>/Excel
    by Name/<Name>.xlsx). اگر فایل آن Name وجود نداشته باشد، با هدر مناسب
    ساخته می‌شود؛ اگر وجود داشته باشد، فقط ردیف‌های جدید به پایین آن اضافه
    می‌شوند و محتوای قبلی دست‌نخورده باقی می‌ماند."""
    if not entries:
        return []
    ...
    try:
        ...
        messagebox.showerror(
            "Dispense Report",
            "Excel file could not be created because the 'openpyxl' package is not installed.\n\n"
            "Install it with: pip install openpyxl\n"
            "(The .txt report was still saved successfully.)"
        )
        return []

    excel_dir = os.path.join(report_dir, EXCEL_BY_NAME_SUBFOLDER)
    try:
        os.makedirs(excel_dir, exist_ok=True)
    except Exception as ex:
        messagebox.showerror("Dispense Report", f"Could not create Excel folder:\n{ex}")
        return []

    # گروه‌بندی لیبل‌های تازه بر اساس Name، به همان ترتیبی که پرینت شده‌اند
    grouped = {}
    order = []
    for entry in entries:
        fields = _parse_label_fields(entry)
        name = fields["Name"].strip() or "Unknown"
        if name not in grouped:
            grouped[name] = []
            order.append(name)
        grouped[name].append(fields)

    updated_paths = []

    for name in order:
        # نام فایل را از کاراکترهای غیرمجاز در نام فایل ویندوز پاک می‌کند
        safe_name = re.sub(r'[\\/:*?"<>|]', "_", name) or "Unknown"
        ...
        if os.path.exists(excel_path):
            ...
            ws.append(_EXCEL_COLUMN_HEADERS)

        for fields in grouped[name]:
            row_number = ws.max_row  # ردیف 1 هدر است، پس این برابر تعداد رکوردهای فعلی است
            ws.append([
                row_number,
                fields["Name"],
                fields["BachNo"],
        ...
        # عرض ستون‌ها را متناسب با محتوا تنظیم می‌کند تا خوانا باشد
        for col_idx, header in enumerate(_EXCEL_COLUMN_HEADERS, start=1):
            max_len = len(header)
        ...
        wb.save(excel_path)
        updated_paths.append(excel_path)
...
def generate_dispense_report():
    ...
    هیچ اطلاعاتی از batch(های) قبلی در آن نمی‌آید. (این رفتار txt بدون تغییر
    باقی مانده است.)

    علاوه بر txt، به‌ازای هر Name متفاوتِ روی لیبل، یک فایل .xlsx دائمی در
    '<Dispense report>/Excel by Name/<Name>.xlsx' ساخته/به‌روز می‌شود که هیچ‌وقت
    از نو ساخته یا بازنویسی نمی‌شود — فقط لیبل‌های تازه (که از دفعه‌ی قبلِ زدن
    دکمه‌ی Report چاپ شده‌اند) به انتهای همان فایل اضافه می‌شوند.
    """
    ...
    try:
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            for entry in entries:
                # برای لیبل‌های QC، خط Time طوری اصلاح می‌شود که فقط ساعت اول
                # نوشته شود (مثلاً 'Time: 14:36' به‌جای 'Time: 14:36 - --:--')
                f.write(_format_entry_for_txt(entry))
                f.write("\n" + ("-" * 30) + "\n")
    except Exception as ex:
        messagebox.showerror("Dispense Report", f"Could not write report file:\n{ex}")
        return

    # --- افزودن فقط لیبل‌های تازه (که از دفعه‌ی قبلِ زدن Report برای این batch
    # اضافه شده‌اند) به فایل اکسل دائمی مربوط به Name همان لیبل‌ها. برخلاف txt،
    # این فایل‌ها هیچ‌وقت بازنویسی/از نو ساخته نمی‌شوند — فقط رکورد جدید به
    # آخرشان اضافه می‌شود، تا با کلیک‌های بعدی دکمه‌ی Report روی همین batch،
    # لیبل‌های قبلی دوباره و تکراری درج نشوند.
    already_exported = _excel_export_progress_by_batch.get(batch_name, 0)
    new_entries = entries[already_exported:]
    try:
        excel_paths = _append_entries_to_name_excel(report_dir, new_entries)
        _excel_export_progress_by_batch[batch_name] = len(entries)
    except Exception as ex:
        excel_paths = []
        messagebox.showerror("Dispense Report", f"Could not update Excel report file(s):\n{ex}")
...
def open_print_settings_window():
    ...
    Label(settings_win, text="Connected printer:", bg='#505a7a', font=UI_TOPBAR_FONT).grid(
    ...
    def add_setting_row(r, label_text, initial_value):
        Label(settings_win, text=label_text, bg='#505a7a', font=UI_TOPBAR_FONT).grid(
    ...
    e_copies = add_setting_row(7, "Copies per Print:", PRINT_COPIES_PER_CLICK)

    status_lbl = Label(settings_win, text="", bg='#505a7a', fg="#C0392B",
                       font=("Segoe UI", 8), wraplength=320, justify='left')
    status_lbl.grid(row=8, column=0, columnspan=3, padx=6, pady=(2, 0), sticky='w')
    ...
    def save_settings():
        ...
        try:
            ...
            new_copies = int(e_copies.get())
        except ValueError:
            status_lbl.config(text="Margins, font size, and copies must be numbers.")
            return
        if new_copies < 1:
            status_lbl.config(text="Copies per Print must be at least 1.")
            return
        ...
        PRINT_COPIES_PER_CLICK = new_copies
        settings_win.destroy()

    btn_row = Frame(settings_win, bg='#505a7a')
    btn_row.grid(row=9, column=0, columnspan=3, pady=(8, 10))
...
def open_advance_window():
    ...
    adv_win.configure(bg="#505a7a")
    adv_win.resizable(False, False)
    adv_win.transient(window)
    try:
        adv_win.iconbitmap(resource_path("barlogo.ico"))
    except Exception:
        pass

    Label(adv_win, text="Product list (Name : BachNo prefix):", bg='#505a7a',
    ...
    Label(adv_win, text="Name:", bg='#505a7a', font=UI_TOPBAR_FONT).grid(
    ...
    Label(adv_win, text="Letter:", bg='#505a7a', font=UI_TOPBAR_FONT).grid(
    ...
    status_lbl = Label(adv_win, text="", bg='#505a7a', fg="#C0392B",
    ...
    def delete_product():
        p_name = e_name.get().strip()
        if not p_name:
            status_lbl.config(text="Select an item from the list (or type its Name) to delete.")
            return
        if p_name not in PRODUCTS:
            status_lbl.config(text=f"'{p_name}' is not in the list.")
            return
        if len(PRODUCTS) <= 1:
            # همیشه باید حداقل یک محصول باقی بماند — چند جای دیگر کد فرض می‌کنند
            # PRODUCTS هیچ‌وقت کاملاً خالی نیست.
            status_lbl.config(text="Cannot delete the last remaining product.")
            return
        del PRODUCTS[p_name]
        if not save_products(PRODUCTS):
            status_lbl.config(text="Could not save to disk (this session's list is still updated).")
        else:
            status_lbl.config(text=f"Deleted '{p_name}'.")
        e_name.delete(0, "end")
        e_letter.delete(0, "end")
        refresh_list()
        refresh_dropdown()
        # اگر همان محصولی که در دراپ‌داون پایین صفحه انتخاب شده بود حذف شد، انتخاب
        # را به یکی از محصولات باقی‌مانده تغییر بده تا دکمه‌ی Apply خراب نشود.
        if select_var.get() == p_name:
            select_var.set(next(iter(PRODUCTS.keys()), ""))

    Button(adv_win, text="🗑 Delete", bg="#C0392B", fg="white",
          command=delete_product).grid(row=2, column=5, padx=6, pady=4)

    # --------- انتخاب محصول برای اعمال روی لیبل پرینتر ---------
    Label(adv_win, text="Select for print label:", bg='#505a7a', font=UI_TOPBAR_FONT).grid(
    ...
    adv_btn_row = Frame(adv_win, bg='#505a7a')
    ...
    Button(adv_btn_row, text="Close", width=12, bg="#D4250E",command=adv_win.destroy).grid(row=0, column=1, padx=4)
...
welcome_frame = Frame(window, bg="#617cd4")
...
_SPLASH_CX = FIT_WINDOW_WIDTH / 6.3
_SPLASH_CY = FIT_WINDOW_HEIGHT / 4.1   # دقیقاً وسط صفحه (هم افقی هم عمودی)
_SPLASH_BASE_R = min(FIT_WINDOW_WIDTH, FIT_WINDOW_HEIGHT) * 0.11   # چند برابر بزرگ‌تر از نسخه‌ی قبلی
...
# -----------------------------------------
# دکمه Clear صفحه‌ی اصلی این پنجره را باز می‌کند (دیگر با START اجرا نمی‌شود)

def _select_daily_batch_number():
    """با زدن دکمه‌ی Clear، این پنجره باز می‌شود و از کاربر می‌خواهد عددی بین
    1 تا 10 را برای batch امروز انتخاب کند. عدد انتخاب‌شده در DAILY_BATCH_NUMBER
    ذخیره و بلافاصله کل بخش BachNo (پیشوند + تاریخ امروز + همین شماره) روی
    لیبل به‌روزرسانی می‌شود."""
    global DAILY_BATCH_NUMBER

    picker = Toplevel(window)
    picker.title("Batch Number")
    picker.configure(bg='#505a7a')
    picker.resizable(False, False)
    picker.transient(window)
    picker.grab_set()
    try:
        picker.iconbitmap(resource_path("barlogo.ico"))
    except Exception:
        pass

    Label(picker, text="Enter the number of bach for today", bg='#505a7a', fg='white',
         font=UI_TOPBAR_FONT).grid(row=0, column=0, columnspan=5, padx=14, pady=(14, 10))

    def choose(n):
        global DAILY_BATCH_NUMBER
        DAILY_BATCH_NUMBER = n
        _apply_daily_batch_number_to_label()
        picker.grab_release()
        picker.destroy()
    ...
    for i in range(1, 11):
        r = 1 + (i - 1) // 5
        c = (i - 1) % 5
        Button(picker, text=str(i), width=4, bg="#D5DBDB",
              command=lambda n=i: choose(n)).grid(row=r, column=c, padx=4, pady=4)

    # اگر کاربر پنجره را با ضربدر ببندد (بدون انتخاب عدد)، همان مقدار پیش‌فرض
    # 1 (یعنی -01) اعمال می‌شود تا برنامه هیچ‌وقت گیر نکند.
    picker.protocol("WM_DELETE_WINDOW", lambda: choose(1))

    picker.update_idletasks()
    px = window.winfo_x() + (window.winfo_width() - picker.winfo_reqwidth()) // 2
    py = window.winfo_y() + (window.winfo_height() - picker.winfo_reqheight()) // 2
...
START_BTN_Y = int(FIT_WINDOW_HEIGHT * (550 / 920))
...
# لیست ورودی EID (مقدار پایه‌ی هر مرکز در فرمول اکتیویته، پیش‌فرض 10)----------

listEID=[entryEID1,entryEID2,entryEID3,entryEID4,
entryEID5,entryEID6,entryEID7,entryEID8,entryEID9,entryEID10,entryEID11,entryEID12]
# ردیف لیبل مراکز
