...
class ChamberReading:
    chamber_id: str
    isotope: str
    field3_raw: str           # فیلد سوم خام — دیگر برای activity استفاده نمی‌شود (قبلاً به‌اشتباه استفاده می‌شد)
    activity: float           # بر حسب میکروکوری (uCi) است، نه میلی‌کوری — از فیلد پنجم (index 4) گرفته می‌شود؛ قبل از نمایش باید بر 1000 تقسیم شود
    ...
    def parse(raw_line: str) -> "ChamberReading":
        ...
        # ترتیب فیلدها: 1=chamber_id، 2=isotope، 3=field3_raw (بلااستفاده)،
        # 4=field4/نیمه‌عمر، 5=activity (طبق توافق، همین فیلد پنجم است)،
        # 6=counter، 7=trailing.
        chamber_id, isotope, field3_raw, field4, activity_str, counter_str, trailing = parts[:7]
        return ChamberReading(
            chamber_id=chamber_id,
            isotope=isotope,
            field3_raw=field3_raw,
...
def comecer_update_all_chambers():
    ...
    for chamber_id, baseline in comecer_chamber_baselines.items():
        ...
        if chamber_id == CHAMBER_ID_FOR_VIK202 and not comecer_state["activity_locked"]:
