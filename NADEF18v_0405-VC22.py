    ...
    def _split_lines(self, buffer: bytes):
        ...
        if term == "auto":
            for t in (b"\r\n", b"\n", b"\r"):
                ...
                # پروتکل واقعی Comecer هیچ کاراکتر پایان‌خطی (\r\n) نمی‌فرستد؛
                # هر رکورد با طول ثابت ارسال می‌شود (همان‌قدر که در تنظیمات
                # خودِ Comecer هم به‌عنوان bytes_to_read مشخص شده، مثلاً 50 بایت).
                # بنابراین وقتی هیچ جداکننده‌ای پیدا نشود، بافر را به بلوک‌های
                # هم‌طول تقسیم می‌کنیم؛ در غیر این صورت داده هیچ‌وقت پردازش
                # نمی‌شد و فقط در بافر انباشته می‌ماند.
                record_len = int(self.config.get("bytes_to_read", 50) or 50)
                if record_len <= 0 or len(buffer) < record_len:
                    return [], buffer
                n_complete = len(buffer) // record_len
                complete = buffer[: n_complete * record_len]
                remainder = buffer[n_complete * record_len:]
...
def comecer_handle_reading(reading):
    ...
    is_match = (selected == "" or selected.lower() == "all" or reading.chamber_id == selected)
    if not is_match:
        return
    # «Live:» فقط برای چمبر انتخاب‌شده (یا حالت all) به‌روزرسانی می‌شود؛ در غیر این
    # صورت وقتی چند چمبر پشت‌سرهم داده می‌فرستند، این مقدار هر لحظه بین چمبرهای
    # مختلف در حال تعویض/چشمک‌زدن بود.
