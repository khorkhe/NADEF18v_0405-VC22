...
# دکمه Report — طبق درخواست، دقیقاً هم‌ردیف با دکمه Connect (ردیف بالای پنل
# Comecer) قرار می‌گیرد؛ چون قبلاً بدون sticky='n' وسط کل ارتفاع frame_comecer
# (که ۳ ردیف داخلی دارد) قرار می‌گرفت، نه دقیقاً کنار Connect.
...
btn_report.grid(row=1, column=9, padx=6, pady=(6, 0), sticky='n')
...
# مرجع (baseline) افت پرتوزایی هر چمبر — با داده‌ی واقعی لاگ ثابت شد که مقدار
# activity خام یک عدد مرجع/کالیبراسیون است، نه یک مقدار زنده‌ی لحظه‌به‌لحظه؛
# پس این مرجع را نگه می‌داریم و comecer_tick_display هر ثانیه با فرمول افت
# پرتوزایی آن را به لحظه‌ی حال تصحیح می‌کند.
...
def comecer_handle_reading(reading):
    ...
    comecer_log(f"PARSED: chamber={reading.chamber_id} isotope={reading.isotope} "
                f"raw_activity={reading.activity} -> {activity_mci:.3f} mCi "
                f"counter={reading.refresh_counter} raw_line={reading.raw_line!r}")

    # --- جدول RAW: متن خام زنده، مستقل از این‌که مقدار activity تغییر کرده یا نه ---
    ...
    # --- مرجع (baseline) افت پرتوزایی: با لاگ واقعی ثابت شد که مقدار خام
    # activity یک عدد مرجع/کالیبراسیون است که در بازه‌های طولانی (حدود ساعت‌ها،
    # نه ثانیه) تغییر می‌کند، نه هر ثانیه — در حالی که صفحه‌ی زنده‌ی خودِ دستگاه
    # (IBC-LITE) به‌صورت پیوسته افت پرتوزایی را کم می‌کند. پس این مرجع را فقط
    # وقتی مقدار خام واقعاً عوض شود بازنشانی می‌کنیم؛ نمایش واقعی روی صفحه را
    # comecer_tick_display (هر ثانیه) با اعمال افت پرتوزایی به لحظه‌ی حال انجام می‌دهد.
    ...
    if prev is None or abs(prev["activity_mci"] - activity_mci) > 1e-6:
        ...
        comecer_log(f"NEW BASELINE: chamber={reading.chamber_id} activity={activity_mci:.3f} mCi "
                    f"(مقدار خام واقعاً عوض شد؛ مرجع افت پرتوزایی بازنشانی شد)")

    comecer_update_all_chambers()   # نمایش را همین لحظه هم به‌روزرسانی کن (نه فقط منتظر تیک بعدی)


def comecer_decayed_now(baseline):
    """فعالیت باسلاین را با گذشت زمان (از لحظه‌ی ثبت مرجع تا الان) به لحظه‌ی
    حال تصحیح می‌کند: A(t) = A0 × 0.5^(t / نیمه‌عمر)."""
    if baseline is None:
        return None
    ...
    return baseline["activity_mci"]


def comecer_update_all_chambers():
    """برای هر دو چمبر، عدد افت‌پرتوزایی‌تصحیح‌شده را در جدول Activity به‌روز
    می‌کند. طبق درخواست: entry0 (Whole Activity) همیشه از VIK-202 پر می‌شود؛
    خط Activity: در لیبل چاپ همیشه از VIK-203 پر می‌شود — مگر این‌که دکمه‌ی
    Stop زده شده باشد، که در آن صورت خط Activity: کاملاً ثابت می‌ماند."""
    for chamber_id, baseline in comecer_chamber_baselines.items():
        activity_now = comecer_decayed_now(baseline)
        if activity_now is None:
            continue
        isotope = baseline["isotope"]

        if chamber_id in comecer_activity_vars:
            comecer_activity_vars[chamber_id].set(f"{activity_now:.3f} mCi  ({isotope})")

        if chamber_id == CHAMBER_ID_FOR_VIK202:
            # ظرف بالک/مادر → مستقیم و پیوسته در entry0 (Whole Activity) نوشته می‌شود.
            entry0.delete(0, "end")
            entry0.insert(0, f"{activity_now:.3f}")

        if chamber_id == CHAMBER_ID_FOR_VIK203 and not comecer_state["paused"]:
...
def comecer_tick_display():
    """این تابع فقط توسط خودِ چرخه‌ی window.after هر ثانیه صدا زده می‌شود (نه از
    داخل comecer_handle_reading) — تا فقط یک زنجیره‌ی زمان‌بندی همیشه فعال باشد،
    نه یکی برای هر رکورد ورودی."""
    comecer_update_all_chambers()
...
def comecer_toggle_hold():
    ...
    if comecer_state["paused"]:
        # هنگام توقف، مقدار افت‌پرتوزایی‌تصحیح‌شده‌ی همین لحظه‌ی VIK-203 (نه مقدار
        # خام آخرین رکورد دریافتی، که ممکن بود اصلاً مربوط به چمبر دیگری باشد)
        # را ثابت در خط Activity: نگه می‌داریم.
        baseline = comecer_chamber_baselines.get(CHAMBER_ID_FOR_VIK203)
        activity_now = comecer_decayed_now(baseline)
