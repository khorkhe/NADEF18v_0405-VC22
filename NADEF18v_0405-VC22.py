...
def update_batch_line(event):
    ...
    if value:
        ...
        batch_line = None
        for i, line in enumerate(lines):
            if line.startswith("BachNo:C020"):
                batch_line = i
                break
        if batch_line is None:
            ...
            month = today.month
            day = today.day
            batch_number = f"BachNo:C020{year:02d}{month:02d}{day:02d}-01/"
...
class ChamberReading:
    chamber_id: str
    isotope: str
    activity: float          # بر حسب میکروکوری (uCi) است، نه میلی‌کوری — قبل از نمایش باید بر 1000 تقسیم شود
...
class ComecerReader:
    ...
    # شروع هر رکورد همیشه با «دو رقم + کاما + حرف» است، مثل '01,F' یا '02,F'.
    # چون طول واقعی فیلدها (مثلاً شمارنده وقتی از ۳ رقم به ۴ رقم برسد) می‌تواند
    # کمی تغییر کند، به‌جای فرض یک طول ثابت (که باعث ناهم‌ترازی و از دست رفتن
    # اکثر رکوردهای واقعی می‌شد)، این الگو را برای پیدا کردن مرز هر رکورد
    # استفاده می‌کنیم — مستقل از طول دقیق فیلدها و خودهمگام‌شونده (self-syncing).
    _RECORD_START_RE = re.compile(rb'\d{2},[A-Za-z]')
    ...
    def _split_lines(self, buffer: bytes):
        ...
        if term == "auto":
            for t in (b"\r\n", b"\n", b"\r"):
                ...
                # پروتکل واقعی Comecer نه کاراکتر پایان‌خط دارد و نه طول ثابت
                # واقعی (نمونه‌های مشاهده‌شده ۴۵ بایت بودند، نه ۵۰ که قبلاً فرض
                # شده بود — همین اختلاف باعث ناهم‌ترازی و پارس نادرست/گم‌شدن
                # اکثر رکوردهای واقعی می‌شد). بنابراین شروع هر رکورد بعدی را
                # پیدا می‌کنیم و هرچه بین دو شروع پیاپی است را یک رکورد کامل
                # در نظر می‌گیریم؛ آخرین قطعه (بعد از آخرین شروع) در بافر
                # نگه داشته می‌شود چون ممکن است هنوز کامل نرسیده باشد.
                positions = [m.start() for m in self._RECORD_START_RE.finditer(buffer)]
                if len(positions) < 2:
                    return [], buffer
                complete_lines = [buffer[positions[i]:positions[i + 1]]
                                   for i in range(len(positions) - 1)]
                remainder = buffer[positions[-1]:]
...
# دکمه Settings — در ستون تازه (9)، بعد از همه‌ی اجزای موجود frame0، تا هیچ‌کدام از
# محل/اندازه‌ی سایر ویجت‌های frame0 و frame1 (که با آن هم‌عرض شده) جابه‌جا نشود.
btn_settings = Button(frame0, text="\u2699 Settings", fg="#000000", bg="#D5DBDB",
                      font=UI_TOPBAR_FONT, command=lambda: open_print_settings_window())
btn_settings.grid(row=0, column=9, padx=6)

# دکمه Report — دقیقاً زیر دکمه Settings (همان ستون 9، ردیف 1 که در frame0 خالی است؛
# frame_comecer در ردیف 1 فقط ستون‌های 0 تا 8 را با columnspan=9 اشغال می‌کند).
btn_report = Button(frame0, text="\U0001F4C4 Report", fg="#000000", bg="#D5DBDB",
                    font=UI_TOPBAR_FONT, command=lambda: generate_dispense_report())
btn_report.grid(row=1, column=9, padx=6, pady=(2, 0))
...
Label(frame_comecer, text="Chamber:", bg=COMECER_BG, fg=COMECER_FG, font=COMECER_FONT).grid(
    row=0, column=4, padx=2)
comecer_chamber_var = StringVar(value="01")
combo_comecer_chamber = ttk.Combobox(frame_comecer, textvariable=comecer_chamber_var,
                                     values=["01", "02", "all"], width=5, state="readonly")
combo_comecer_chamber.grid(row=0, column=5, padx=2)

comecer_status_var = StringVar(value="Not connected")
comecer_live_var = StringVar(value="---")
comecer_matched_var = StringVar(value="---")
...
comecer_state = {"paused": False}   # وضعیت توقف/ادامه خواندن (برای دکمه کنار Print)

# مرجع (baseline) فعالیت برای هر چمبر: چون مقدار خام دریافتی از پروتکل Comecer
# یک عدد "مرجع ثابت" است که فقط هروقت دستگاه واقعاً اندازه‌گیری تازه انجام دهد
# تغییر می‌کند (نه هر بار که همان رکورد قبلی دوباره روی خط ارسال می‌شود)، این
# دیکشنری آخرین مرجع واقعی هر چمبر را به همراه لحظه‌ی دریافت آن نگه می‌دارد.
# سپس comecer_tick_display هر ثانیه با فرمول افت پرتوزایی، عدد را به لحظه‌ی
# حال تصحیح می‌کند — دقیقاً همان کاری که صفحه‌ی زنده‌ی خود دستگاه IBC-LITE انجام
# می‌دهد. بدون این تصحیح، عدد نمایشی ثابت می‌ماند و به‌مرور با عدد واقعی دستگاه
# مغایرت پیدا می‌کند (چون آن‌جا به‌صورت زنده افت می‌کند ولی این‌جا نه).
comecer_chamber_baselines = {}   # chamber_id -> {"activity_mci","isotope","half_life_min","since"}
...
def comecer_handle_reading(reading):
    """فقط مرجع (baseline) هر چمبر را به‌روز نگه می‌دارد؛ نمایش واقعی روی صفحه
    توسط comecer_tick_display (هر ثانیه) با اعمال افت پرتوزایی زنده انجام می‌شود."""
    comecer_reader_holder["last_reading"] = reading
    comecer_reader_holder["last_chamber_seen"] = reading.chamber_id
    activity_mci = reading.activity / 1000.0   # مقدار خام بر حسب uCi است -> تبدیل به mCi
    prev = comecer_chamber_baselines.get(reading.chamber_id)
    if prev is None or abs(prev["activity_mci"] - activity_mci) > 1e-6:
        # مقدار خام واقعاً تغییر کرده (یا اولین‌بار است) => این لحظه مرجع تازه است
        comecer_chamber_baselines[reading.chamber_id] = {
            "activity_mci": activity_mci,
            "isotope": reading.isotope,
            "half_life_min": reading.half_life_minutes,
            "since": reading.timestamp,
        }
    # اگر مقدار خام همان قبلی بود، عمداً baseline را دست‌نخورده می‌گذاریم تا
    # محاسبه‌ی افت پرتوزایی پیوسته بماند (نه اینکه هر بار از نو صفر شود).


def comecer_decayed_now(baseline):
    """فعالیت باسلاین را با گذشت زمان به لحظه‌ی حال تصحیح می‌کند."""
    if baseline is None:
        return None
    half_life = baseline["half_life_min"]
    elapsed_min = (datetime.now() - baseline["since"]).total_seconds() / 60.0
    if half_life and half_life > 0:
        return baseline["activity_mci"] * (0.5 ** (elapsed_min / half_life))
    return baseline["activity_mci"]   # اگر نیمه‌عمر نامعتبر بود، بدون تصحیح


def comecer_tick_display():
    """هر ثانیه اجرا می‌شود و Live/Sel/خط Activity را با عدد تصحیح‌شده به‌روز می‌کند —
    دقیقاً هم‌گام با نحوه‌ی نمایش زنده‌ی خودِ صفحه دستگاه IBC-LITE."""
    selected = comecer_chamber_var.get().strip()
    if selected == "" or selected.lower() == "all":
        chamber_id = comecer_reader_holder.get("last_chamber_seen")
    else:
        chamber_id = selected
    baseline = comecer_chamber_baselines.get(chamber_id) if chamber_id else None
    activity_now = comecer_decayed_now(baseline)
    if activity_now is not None:
        isotope = baseline["isotope"]
        comecer_live_var.set(f"{activity_now:.3f} mCi  ({isotope})  Ch:{chamber_id}")
        comecer_matched_var.set(f"{activity_now:.3f} mCi  ({isotope})")
        if not comecer_state["paused"]:
            comecer_update_activity_line(format_activity_mci(activity_now))
    window.after(1000, comecer_tick_display)


def comecer_show_message(text, color="#FF6B6B"):
    """نمایش وضعیت/خطای اتصال Comecer در نوار وضعیت اختصاصی خود این پنل (comecer_msg_label)
    — کاملاً جدا از کادر گزارش/چاپ (text_field) و از wlabel، تا هرگز زیر آن‌ها پنهان نشود.
    طول متن همیشه اینجا محدود می‌شود (نه فقط در محل تولید پیام) تا هیچ پیامی — هرچقدر
    هم طولانی یا بدون فاصله باشد — نتواند عرض ستون را زیاد کند و دکمه‌ی Report/Settings
    را از کادر برنامه بیرون بِکشد."""
    text = str(text)
    if len(text) > 120:
        text = text[:120] + "..."
    comecer_msg_label.config(text=text, fg=color)
...
def comecer_toggle_connect():
    if comecer_reader_holder["reader"] is None:
        ...
        btn_comecer_connect.config(text="Disconnect", bg="#C0392B", fg="white")
    else:
        ...
        btn_comecer_connect.config(text="Connect", bg="#1E8449", fg="white")


btn_comecer_connect = Button(frame_comecer, text="Connect", bg="#1E8449", fg="white",
                             width=10, command=comecer_toggle_connect)
btn_comecer_connect.grid(row=0, column=6, padx=6)

Label(frame_comecer, text="Live:", bg=COMECER_BG, fg=COMECER_FG, font=COMECER_FONT).grid(
    row=0, column=7, padx=(4, 2), pady=2, sticky='w')
Label(frame_comecer, textvariable=comecer_live_var, bg=COMECER_BG, fg=COMECER_VALUE_FG,
      font=COMECER_VALUE_FONT).grid(row=0, column=8, pady=2, sticky='w')

Label(frame_comecer, text="Sel:", bg=COMECER_BG, fg=COMECER_FG, font=COMECER_FONT).grid(
    row=1, column=0, padx=(6, 2), pady=1, sticky='w')
Label(frame_comecer, textvariable=comecer_matched_var, bg=COMECER_BG, fg=COMECER_VALUE_FG,
      font=COMECER_VALUE_FONT).grid(row=1, column=1, columnspan=2, pady=1, sticky='w')

# نوار وضعیت/خطا — یک خط فشرده، جدا از کادر گزارش/چاپ
comecer_msg_label = Label(frame_comecer, text="Not connected", bg="black", fg="#7EC8FF",
                          font=("Segoe UI", 8), anchor='w', justify='left', wraplength=860)
comecer_msg_label.grid(row=1, column=3, columnspan=6, sticky='ew', padx=4, pady=1)
...
def get_persian_date_code():
    try:
        ...
        return "BC020000000-01"  # fallback if package not installed
    today = JalaliDate.today()
    year = today.year % 100  # last two digits
    month = today.month
    day = today.day
    return f"BachNo:C020{year:02d}{month:02d}{day:02d}-01/"
...
# Insert initial text
text_field.insert("1.0", f"Name:FDG\nActivity: 0 mCi\nA-Activity: 0 mCi\n{persian_date}\n15 ml container/ ml\n" + get_persian_date_code())
...
PRINT_MARGIN_TOP_IN = 0.2
PRINT_MARGIN_LEFT_IN = 0.4
...
def comecer_toggle_hold():
    ...
    if comecer_state["paused"]:
        # هنگام توقف، آخرین مقدار دریافتی را ثابت در خط Activity: نگه می‌داریم
        last = comecer_reader_holder["last_reading"]
        if last is not None:
            comecer_update_activity_line(format_activity_mci(last.activity / 1000.0))
