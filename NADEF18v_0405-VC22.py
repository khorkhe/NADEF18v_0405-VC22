...
CHAMBER_ID_FOR_VIK203 = "01"   # ظرف دوز بیمار (Patient dose vial) → entry0 (Whole Activity)
CHAMBER_ID_FOR_VIK202 = "02"   # ظرف بالک/مادر (Bulk/mother vial) → لیبل جداگانه‌ی VIK-202
...
combo_comecer_chamber = ttk.Combobox(frame_comecer, textvariable=comecer_chamber_var,
                                     values=["01", "02"], width=5, state="readonly")
...
comecer_vik202_var = StringVar(value="---")   # نمایش جداگانه‌ی ظرف بالک/مادر (VIK-202)
...
def comecer_handle_reading(reading):
    ...
    # نکته‌ی مهم: مقدار خامی که از پروتکل Comecer می‌رسد بر حسب میکروکوری (uCi) است،
    # نه میلی‌کوری — برای همین قبل از نمایش/استفاده به mCi تبدیل می‌شود (تقسیم بر 1000).
    activity_mci = reading.activity / 1000.0

    # --- مسیر خودکار و همیشگی، بر اساس هویت فیزیکی ظرف (نه انتخاب دستی dropdown) ---
    if reading.chamber_id == CHAMBER_ID_FOR_VIK203:
        # ظرف دوز بیمار → مستقیم در entry0 (Whole Activity) نوشته می‌شود.
        entry0.delete(0, "end")
        entry0.insert(0, f"{activity_mci:.3f}")
    elif reading.chamber_id == CHAMBER_ID_FOR_VIK202:
        # ظرف بالک/مادر → در لیبل جداگانه‌ی خودش نمایش داده می‌شود، نه entry0.
        comecer_vik202_var.set(f"{activity_mci:.3f} mCi  ({reading.isotope})")

    # --- مسیر قبلی (دستی): همچنان برای Live:/Sel: و خط Activity در لیبل پرینت،
    # بر اساس چمبر انتخاب‌شده در dropdown، به‌عنوان قابلیت جداگانه حفظ شده است ---
    selected = comecer_chamber_var.get().strip()
    is_match = (selected == "" or reading.chamber_id == selected)
    if not is_match:
        return
    comecer_live_var.set(f"{activity_mci:.3f} mCi  ({reading.isotope})  Ch:{reading.chamber_id}")
    comecer_matched_var.set(f"{activity_mci:.3f} mCi  ({reading.isotope})")
...
Label(frame_comecer, text="VIK-202 (Bulk):", bg=COMECER_BG, fg=COMECER_FG, font=COMECER_FONT).grid(
    row=2, column=0, padx=(6, 2), pady=1, sticky='w')
Label(frame_comecer, textvariable=comecer_vik202_var, bg=COMECER_BG, fg=COMECER_VALUE_FG,
      font=COMECER_VALUE_FONT).grid(row=2, column=1, columnspan=2, pady=1, sticky='w')
