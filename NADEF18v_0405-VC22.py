...
class ChamberReading:
    chamber_id: str
    isotope: str
    activity: float          # mCi طبق نمونه‌های مشاهده‌شده در نرم‌افزار اصلی
...
def comecer_handle_reading(reading):
    ...
    comecer_live_var.set(f"{reading.activity:.3f} mCi  ({reading.isotope})  Ch:{reading.chamber_id}")
    selected = comecer_chamber_var.get().strip()
    if selected == "" or selected.lower() == "all" or reading.chamber_id == selected:
        comecer_matched_var.set(f"{reading.activity:.3f} mCi  ({reading.isotope})")
        if not comecer_state["paused"]:
            comecer_update_activity_line(format_activity_mci(reading.activity))
...
def comecer_toggle_hold():
    ...
    if comecer_state["paused"]:
        ...
        if last is not None:
            comecer_update_activity_line(format_activity_mci(last.activity))
