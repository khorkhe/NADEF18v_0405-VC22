...
def apply_qc_suffix(qc_name):
    """با زدن QC1 یا QC2: فقط بخش بعد از '/' در خط BachNo با QC1/QC2 جایگزین
    می‌شود و حجم خط 'container/' روی مقدار پیش‌فرض 0.3 تنظیم می‌شود (که با
    تایپ‌کردن قابل اصلاح است). خطوط Time/Cal Time و Activity/A-Activity دست‌نخورده
    باقی می‌مانند تا هر دو دکمه دقیقاً یک مقدار Time/Cal Time و در نتیجه یک
    Activity/A-Activity یکسان نشان دهند."""
    text = text_field.get("1.0", "end-1c")
    lines = text.split('\n')
